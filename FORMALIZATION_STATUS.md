...
|---|---|---|
| `def:cclass` | 定义与基本性质完成 | `Basic.lean`、`Convergence.lean`、`Hurwitz.lean`：子列保持、主导指标更换、有限抽取与 Hurwitz 非零极限均已证明。归一化商已收敛时可得两个主导指标；原稿的子列版本尚缺 Montel 抽取，仍未计为完成。 |
...
- 当前本地源码包含 102 个已证明定理。它们包含辅助引理，不能把数量当作论文完成比例。
