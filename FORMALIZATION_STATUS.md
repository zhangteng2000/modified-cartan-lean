...
|---|---|---|
| `def:cclass` | 定义与基本性质完成 | `Basic.lean` 与 `Convergence.lean`：固定主导指标；紧集上一致收敛；C-类非空、至少两个指标；限制区域与子列保持；主导指标更换判据；有限同时抽取。连通域上两个主导指标的 Hurwitz 推论未证明。 |
...
- 当前本地源码包含 98 个已证明定理。它们包含辅助引理，不能把数量当作论文完成比例。
