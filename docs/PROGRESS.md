...
Further classical extraction work: the three-function base is now proved on arbitrary nonempty connected open plane domains. OmittedValues proves genuine two-omitted-value normality. The first bounded-logarithmic-derivative branch, pair overlap, index unmerging, isolated-zero extension, annulus construction/topology, and maximum-principle filling are individually compiled. CartanFourReduction proves both initial four-function reduction cases on every interior disk. See CARTAN_EXTRACTION.md; the higher Wronskian branches and full five-function extraction remain WIP.
