...
`WronskianExponents.lean` produces valid exponents for all dimensions at a = 1/4, b = 1/2 with K₁ = 1. `ProximityGrowth.lean` proves pointwise Poisson/proximity comparison and the local bound |h| ≤ exp(1 + 4 m(R,h)/(R-r)) on the closed midpoint disk. The logarithmic-derivative lemma remains WIP; these are its initial analytic estimates, not a replacement for its full statement.
