...
Classical Cartan extraction source: Henri Cartan, Ann. Sci. ENS 45 (1928), Theorem VII, pp. 312-315, DOI 10.24033/asens.786, https://www.numdam.org/articles/10.24033/asens.786/. The original pages have been read; no cited theorem is introduced as a Lean axiom. The required p = 5 extraction is now proved in CartanFive.lean; p = 3 and p = 4 are also proved. The local mathlib complex-analysis Cartan namespace concerns a different integral formula and does not supply this extraction theorem.

Classical extraction preparation: existing holomorphic-log construction, mathlib convex mean-value bounds and the previously proved Montel/Hurwitz/failure-point tools prove LogDerivativeNormality. The finite five-index classification uses mathlib finite-set cardinality; the analytic assembly uses the already proved sharp_two_absorption without a connectedness assumption. These auxiliary proofs are now part of the completed extraction proof.
...
926-declaration audit: finite outer-measure subadditivity and compact shrinking of balls are reused to construct anchors. Iterated derivative congruence on actual neighborhoods handles the constant-sum column replacement. The higher derived-fraction anchor induction and complete growth closure are proved in local Lean modules, not supplied by any external mathematical axiom.

938-declaration audit: finite zero factorization and measure_sdiff_null remove all exceptional zero circles with no measurability assumption on the original radius set. Existing Montel, Hurwitz and overlapping cancellation limits produce the actual bounded-three-index contradiction. No analytic normality conclusion is assumed as an axiom.

948-declaration checkpoint: Matrix.det_permute' and the unit norm of a permutation sign give the normalized-Wronskian permutation identities. Existing finite union measure subadditivity is sufficient for the radial dichotomy; no measurability of projected exceptional sets is assumed. The four-function circle branch reuses the proved derivative means, reciprocal proximity and radial-zero removal.

988-declaration checkpoint: finite_constant_subsequence, strictMono_nat_of_lt_succ and StrictMono.id_le support the actual nested diagonal construction. Compactness of individual continuous functions restores finite initial terms. The standard reciprocal sequence limit supplies the explicit disk exhaustion. Small Fin 3 / Fin 4 permutation existence proofs use kernel-checked decide, never native_decide. All analytical branches use the previously proved genuine Wronskian estimates.

## Final Cartan completion and dependency manifest

The five-function extraction is proved locally from the earlier analytic estimates. DiskStripCover and DiskAnnulusCover use mathlib complex logarithm, exponential, analytic composition and open mapping. CompactLifts proves descent of local boundedness and compact convergence using finite compact-ball covers of an open surjection. The annular four-function theorem is therefore derived from the actual full-disk theorem, rather than assumed for a new domain. The higher five-function branch uses finite uniform triple anchors, the fourth radial alternative and the proved inverse-Wronskian growth closure. CartanFiveLocal eliminates all auxiliary case conditions; CartanDiagonal produces the common strict full-disk subsequence.

The exact Cartan input needed by this manuscript is the p = 5 instance. The historical arbitrary-p extraction theorem is not a premise of the final proof. Every principal and auxiliary theorem is recursively audited by Audit.lean; no theorem from the historical reference is inserted as an axiom.

The final compiler-generated import closure, package revisions, and explicit local references to mathlib declarations are enumerated in ../verification/dependencies.json. The graph deliberately includes transitive imports and tactic infrastructure; logical dependencies are separately checked by ../verification/axiom-summary.json. The full theorem count and build status are in ../verification/result.json. Earlier numbered checkpoints above document the API search history, not incomplete current work.
