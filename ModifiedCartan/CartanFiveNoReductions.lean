...
/-- The complete five-unit branch after excluding the initial quotient-limit
reductions. All higher-Wronskian cases are discharged internally. -/
theorem five_two_classes_of_no_reductions {f : Family 5} {L : ℝ}
    ...
    ∃ φ : ℕ → ℕ, StrictMono φ ∧ ∃ I J : Finset (Fin 5), Disjoint I J ∧
      IsCClass (subsequence f φ) I (disk L) ∧ IsCClass (subsequence f φ) J (disk L) := by
  by_contra hnone
  have hsub : disk L ⊆ disk 1 := ball_subset_ball hL1.le
  obtain ⟨a,φ,hφ,hpairs⟩ := anchored_pairs_after_omitting_one (by norm_num : 2 ≤ 5) hL
    (fun i n => ⟨(hf i n).1.mono hsub,fun z hz => (hf i n).2 z (hsub hz)⟩)
    hno hfinite hnone
  let σ : Equiv.Perm (Fin 5) := Equiv.swap (Fin.last 4) a
  let F : Family 5 := fun i n => f (σ i) (φ n)
  ...
  apply five_anchored_subfamily_impossible hL hL1 hη hηL hA
  ...
  intro n i j hij
  have haway (k : Fin 4) : σ k.castSucc ≠ a := by
    have he : σ (Fin.last 4) = a := Equiv.swap_apply_left _ _
    rw [← he]
    exact σ.injective.ne (Fin.castSucc_ne_last k)
  exact hpairs (N+n) (σ i.castSucc) (σ j.castSucc) (haway i) (haway j)
    (σ.injective.ne ((Fin.castSucc_injective 4).ne hij))
