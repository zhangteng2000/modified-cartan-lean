...
theorem log_growth_error_isLittleO {M : ℕ → ℝ} (hM : Tendsto M atTop atTop) (B : ℝ) :
    (fun n => B * (Real.log (M n) + 1)) =o[atTop] M := by
  have h := (Real.isLittleO_log_id_atTop.add (isLittleO_const_id_atTop (1 : ℝ))).comp_tendsto hM
  simpa only [Function.comp_def, Pi.add_apply, id_eq] using h.const_mul_left B
...
theorem derivative_quotient_growth_bound {η τ : ℝ} (hη : 0 < η) (hηh : η < 1 / 2)
    (hτ : 0 < τ) (k : ℕ) :
    ∃ B : ℝ, 0 ≤ B ∧ ∀ (f : ℂ → ℂ) (r R M : ℝ),
      DifferentiableOn ℂ f (disk 1) → τ ≤ diskSupNorm f η →
      1 / 2 ≤ r → r < R → R < 1 → 1 ≤ M → R - r = 1 / M →
...
theorem derivative_quotient_growth_negligible {η τ : ℝ} (hη : 0 < η) (hηh : η < 1 / 2)
    ...
    (hevent : ∀ᶠ n in atTop, τ ≤ diskSupNorm (f n) η ∧
      1 / 2 ≤ r n ∧ r n < R n ∧ R n < 1 ∧ R n - r n = 1 / M n ∧
      proximityMean (f n) (R n) ≤ 2 * M n) :
    (fun n => proximityMean (fun z => iteratedDeriv k (f n) z / f n z) (r n)) =o[atTop] M := by
  obtain ⟨B, hB, hbound⟩ := derivative_quotient_growth_bound hη hηh hτ k
