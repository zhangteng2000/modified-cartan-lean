...
/-- Unconditional classical extraction for five units on every prescribed
strict interior disk. The full unit-disk diagonal step is separate. -/
theorem cartan_five_local {f : Family 5} (hf : UnitFamily f) (hs : ZeroSum f)
    {L : ℝ} (hL : 0 < L) (hL1 : L < 1) :
    ∃ φ : ℕ → ℕ, StrictMono φ ∧
