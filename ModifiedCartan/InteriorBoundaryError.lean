...
theorem interior_wronskian_boundary_error_negligible {m : ℕ} {η ell r₀ : ℝ}
    (hη : 0 < η) (hηh : η < r₀) (hell : 0 < ell)
    (a : Fin m → ℕ → ℂ → ℂ) (r R M : ℕ → ℝ) (B : ℝ)
    (ha : ∀ i n, DifferentiableOn ℂ (a i n) (disk 1))
    (hr : ∀ n, 0 ≤ r n ∧ r n < 1) (hM : Tendsto M atTop atTop)
    (hevent : ∀ᶠ n in atTop, r₀ ≤ r n ∧ r n < R n ∧ R n < 1 ∧
      R n - r n = 1 / M n ∧ ∀ i,
  ...
  have hi : ∀ i (k : Fin m),
    ...
    intro i k
    apply interior_derivative_quotient_growth_negligible hη hηh hell k (a i) r R M (ha i) hM
